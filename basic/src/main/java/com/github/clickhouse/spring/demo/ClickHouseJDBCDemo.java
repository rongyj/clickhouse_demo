package com.github.clickhouse.spring.demo;
...
public class ClickHouseJDBCDemo implements AutoCloseable{
    ...
    private static final String DB_URL = "jdbc:clickhouse://192.168.56.101:8123/default";
    ...
    public void popularYearRoutes(int year) throws SQLException {
        String query = "SELECT " +
                ...
                "FROM datasets.ontime WHERE Year = ? GROUP BY OriginCityName, DestCityName ORDER BY flights DESC LIMIT 20";
    ...
    public void mostFlightCities() throws SQLException {
        String query = "SELECT OriginCityName, " +
                "count(*) AS flights FROM datasets.ontime " +
    ...
    public void maxDirectionCities() throws SQLException {
        String query = "SELECT OriginCityName, uniq(Dest) AS u " +
                "FROM datasets.ontime GROUP BY OriginCityName ORDER BY u DESC LIMIT 20";
        long time = System.currentTimeMillis();
        try (Statement statement = conn.createStatement()) {
            try (ResultSet rs = statement.executeQuery(query)) {
                Util.printRs(rs);
    ...
    public void seasonFlights() throws SQLException {
        String query = "SELECT " +
                ...
                "    FROM datasets.ontime GROUP BY DestCityName HAVING total > 100000 " +
                ") " +
                "ALL INNER JOIN " +
                "( " +
                "    SELECT DestCityName, Month, count() AS monthly " +
                "    FROM datasets.ontime GROUP BY DestCityName, Month HAVING monthly > 10000 " +
                ") " +
                "USING DestCityName " +
    ...
    public static void main(String[] args) throws Exception {
        ClickHouseJDBCDemo clickHouseJDBCDemo = new ClickHouseJDBCDemo();
        clickHouseJDBCDemo.popularYearRoutes(2015);
        clickHouseJDBCDemo.mostFlightCities();
        clickHouseJDBCDemo.maxDirectionCities();
        clickHouseJDBCDemo.seasonFlights();
    }
}